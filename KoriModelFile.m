...
%                                                                       %
%                                                                       %
% Kori-ULB (The ULB Ice Flow Model) is a two-dimensional finite         %
...
%-----------------------------------------------------------------
%
% Matlab files: KoriModel.m, KoriInputParams.m
%
...
%
% v1.9 (04/2022)
%   Expansion of default values originally in FetishInputParams
%   Corrections on geodynamics
%   Application on MISMIP+ and MISMIP3d geometry: ctr.mismip=1
%   Application to single or mutiple basins of an ice sheet: ctr.basin=1
%   Corrections on velocity field (upstream differences)
%   Major correction on ctr.schoof based on Pollard and DeConto (2020)
%   Removal of reduced solution matrix size for initialization (the whole
%       domain is solved for)
%   Implementation of new initialisation schemes:
%       - inverse=2: 
%           - run initialisation with HySSA
%           - fixed calving front
%           - optimize sliding coefficient for grounded ice
%           - optimize sub-shelf melt rates for ice shelves
%           - optimization based on surface elevation (sn-sn0) for grounded
%               ice
%           - optimization based on ice thickness (H-Ho) for floating ice
%   New parameters for sub-shelf melt rate optimization: ctr.TinvMelt and
%       ctr.HinvMelt
%   Change of initialisation procedure:
%       - Asor of grounded grid cells will be optimized --> calculate 
%           smoothed As --> Asor of floating grid cells will be based on
%           smoothed As at grounding line --> calculate again smooth As
%           based on new Asor
%   For inverse runs: Asor=As only if Asor doesn't already exist
%   New definition of ncor
%   New regularization function
%   Implementation of invmax2D that reduces the optimization in the ncor
%       regions to avoid unrealistic behaviour in mountainous regions:
%       invmax2D=par.invmax (default=1e-3) if ncor=1 and
%       invmax2D=par.invmaxncor (default=1e-5) if ncor=0 (further reduced 
%           in high stdB area)
%   Change of grounding line flux calculation 
%       - Take into account orientation of GL [Pollard & DeConto (2020)]
%       --> New function normcalc (to calculate angle of GL)
%       --> New ctr. parameter ctr.radnorm needed
%           - defines radius used to calculate GL direction
%           - defined in km
%   Tolerances and max. iteration number for iterative solver defined as par.
%       - par.veliter, par.veltol, par.Hiter, par.Htol, par.visctol
%       - Decrease tolerance in velocity solver 
%           --> veltol=1e-4 (instead 1e-3)
%           --> veltol=1e-5 (if basin=1)
%   New parameter ctr.ItSolv: use / don't use iterative solver
%   Coulomb friction removed (only Weertman)
%   Only option schoof=1 (schoof=2 (Tsai) removed)
%   General slip law (viscous - power law - Coulomb plastic) [Pattyn, 2021]
%       - new ctr parameter ctr.u0
%       - schoof=2: plastic Coulomb, according to Pattyn et al. (2021)
%   Moved temperature (tmp, Tb) and subglacial sliding (As, Asf) to h-grid
%       instead of d-grid
%   Temperature according to Ritz/Lliboutry only (removal of Tmean)
%   Overall reorganization of model code with subroutines
%   New PDD model
%------------------------------------------------------------------------
...
[ctr,fc]=init_ctr(ctr,fc,default);
...
[Asor,stdB,v,vx,vy,tmp,Db,To,So,Tb,uxssa,uyssa,deltaZ,arcocn,arcocn0, ...
    Pr,Evp,runoff,MeltInv,lat,acc,Smelt,rain,TF,HAF,Hinit,ZB]=deal(false);
...
[ctr.snapshot,plotst,cnt_atm,snp_atm,cnt_ocn,snp_ocn, ...
    ...
    CMB,FMB,flw,p,px,py,pxy,nodeu,nodev,Tof,Sof,TFf,Tsf,Mbf,Prf,Evpf,runofff] ...
    =init_matrices(ctr,par,default,fc);
...
[As,Mb0,Ts0,MASK0,MASK,bMASK,H,B,H0,B0]= ...
    init_input_data(ctr,par,As,Mb,Ts,MASK,MASKo,H,B);
...
[ctr,invmax2D,Asor,ncor,To,So,Pr0,Evp0,runoff0,Evp,Hinit]= ...
    exist_params(ctr,par,ncor,Asor,stdB,v,uxssa,To,So,Db,B,MASK,As, ...
...
if (ctr.schoof>=1 || ctr.shelf==1) && ctr.inverse~=1 %VL: adapted for new inversions
    [HAF,MASK,~,sn]=floatation(par,B,SLR,H,MASK);
else % keep grounding line fixed
    [sn,~]=fixed_floatation(par,B,SLR,H,MASK);
end
...
cntT=0;
if ctr.Tcalc>=1
    [tmp,Tb,zeta,dzc,dzp,dzm]=init_temp_params(ctr,par,tmp,Ts);
end
...
if ctr.GeoidCalc==1
    [Pg0,geoide,frg]=init_geoid(par,ctr,MASK,H0,B0,B,SLR);
end
...
if ctr.BedAdj==1
    [B0,load0,frb,kei,Ll]=bedrock_init(ctr,par,H,B,SLR,MASK);
end
if ctr.BedAdj==2
...
%------------------------------------------------------
    ...
    if (ctr.schoof>=1 || ctr.shelf==1) && ctr.inverse~=1 %VL: adapted for new inversions
        ...
        [sn,HB]=fixed_floatation(par,B,SLR,H,MASK);
    end
...
%------------------------------------------------------
    ...
    [Tsf,Mbf,Prf,Evpf,runofff,cnt_atm,snp_atm,Mb_update]= ...
        ATM_update(fc,ctr,time,cnt,Ts0,Mb0,Pr0,Evp0,runoff0,cnt_atm, ...
    ...
    if Mb_update==0
        [Pr,Evp,runoff]=MbFunc(ctr,Prf,Evpf,runoff,runofff,Tsf,Ts,sn,X,Y,Lj,Li,fc.DeltaT(cnt));
    else
        [Mb,~,~]=MbFunc(ctr,Mbf,Evpf,runoff,runofff,Tsf,Ts,sn,X,Y,Lj,Li,fc.DeltaT(cnt));
    end
    if ctr.PDDcalc==1 % PDD MODEL
        if ctr.monthly==0
            ...
            if rem(time(cnt),1)==0
                [Ts_yc,Pr_yc]=extract_annual_cycle(fc,ctr,par,Pr,Ts,sninit,sn,MASK,lat,fc.DeltaT(cnt),snp_atm);
...
%------------------------------------------------------
    ...
    [gradm,gradmx,gradmy,gradxy,Hm,Hmx,Hmy,Bmx,Bmy,signx,signy]= ...
        staggered_grid(sn,H,B,ctr);
    [bMASKm,bMASKx,bMASKy]=staggered_bMASK(ctr,bMASK);

    % Searching GL on H-grid (define MASK & glMASK)
    [glMASK,MASK]=grounding_line_mask(MASK,bMASK,H,ctr);
...
%--------------------------------------------------------------------------
    ...
    [Asf,Asfx,Asfy,Asfd,Tbc,Neff,pwv,Wtil,r]=basal_sliding(ctr,par, ...
...
%--------------------------------------------------------------------------
    ...
    [A,Ax,Ay,Ad]=thermo_coupling(ctr,par,Tb,Tbc,H,bMASK,bMASKm,bMASKx,bMASKy);
...
%-------------------------------------------------
    ...
    [d,udx,udy,ud,ubx,uby,ub,uxsia,uysia,p,pxy]= ...
        SIA_velocity(ctr,par,A,Ad,Ax,Ay,Asfd,Asfx,Asfy,taud,G,Tb, ...
    ...
    if ctr.Tcalc>=1 % on d-grid
        if cntT==1
            if (ctr.Tinit==1 && cnt==1) || ctr.Tinit==2
                tmp=initTemp3d(G,taudxy,ub,ud,par,H,Mb,zeta,ctr,Ts,MASK,fc.DeltaT(cnt));
            end
            if ctr.Tinit<2
                if ctr.shelf==1 && ctr.SSA>=1 && cnt>1
                    [tmp,ctr]=temperature3d(tmp,Mb,Ts,pxy,par, ...
                        ctr,ctr.dt*par.intT,ud,udx,udy,vec2h(uxssa,uyssa),uxssa, ...
                        uyssa,zeta,gradxy,H,dzc,dzm,dzp,G,taudxy,A,fc.DeltaT,MASK,cnt);
                    Bmelt=basal_melting(ctr,par,G,taudxy,vec2h(uxssa,uyssa),H,tmp,dzm,MASK);
                else
                    [tmp,ctr]=temperature3d(tmp,Mb,Ts,pxy,par, ...
                        ctr,ctr.dt*par.intT,ud,udx,udy,ub,ubx, ...
                        uby,zeta,gradxy,H,dzc,dzm,dzp,G,taudxy,A,fc.DeltaT,MASK,cnt);
                    Bmelt=basal_melting(ctr,par,G,taudxy,ub,H,tmp,dzm,MASK);
                end
            end
        end
...
%----------------------------------------------------------
    ...
    [MASKmx,MASKmy,HAFmx,HAFmy]=SSA_masks(ctr,par,SLR,Bmx,Bmy,Hmx, ...
        Hmy,bMASKx,bMASKy);
...
%-----------------------------------------------
    ...
    if ctr.schoof>=1
       [uxsch,uysch]=grounding_lines(ctr,par,glMASK,HAF,SLR,Ax,Ay,fc.butfac(cnt), ...
            H,Hmx,Hmy,B,Bmx,Bmy,Asfx,Asfy,ux,uy,dudx,dvdy,dudy,dvdx,eta);
    else
        uxsch=ux;
        uysch=uy;
    end
    [uxsch,uysch,d]=diffusive_correction(ctr,par,uxsch,uysch,udx,udy, ...
...
%------------------------------------------------------------------
    ...
    if ctr.glMASKexist==1 && (par.ArcOcean==1 || ctr.calving==1 || ctr.calving==2) && ctr.basin==0
        if cntT==1
...
%------------------------------------------------------
	...
	if ctr.glMASKexist==1 && ctr.inverse==0
		...
		[Tof,Sof,TFf,cnt_ocn,snp_ocn]=OCEAN_update(fc,ctr,time,cnt,So0,To0,Tof,Sof,TFf,cnt_ocn,snp_ocn);

		% Extrapolate To and Tf to the depth of interest according to the chosen melt parameterization
		[To,So,TF]=OCEAN_depth_of_int(fc,ctr,par,Tof,Sof,TFf,H,HB,B,MASK,glMASK,ShelfN,numsh);
	end
...
%---------------------------------------------------------------
    ...
    if ctr.calving>=1 && ctr.shelf==1
        [CMB,LSF,he]=calving_algorithms(ctr,par,dudx,dvdy,dudy,dvdx,glMASK,H,A, ...
            uxssa,uyssa,arcocn,B,runoff,MASK,MASKo,Ho,bMASK,LSF,node,nodes,VM);
        if ctr.calving<5
            [FMB]=vertical_face_melt(ctr,par,SLR,B,Melt,MASK,glMASK,he);
        end
    end
...
%---------------------------------------------------------
    ...
    if ctr.diagnostic==0
        Massb=Mb-Bmelt-Melt-CMB-FMB; % sum of mass balance components for continuity equation
        if ctr.basin==1
            Massb(bMASK==1)=0; % only for ice thickness evolution, not for temperature
        end
        Hn=sparse_solver_new(node,nodes,Massb,H,B,SLR,glMASK,dtdx,dtdx2, ...
...
%----------------------
    ...
    if cntT==1
        if ctr.BedAdj==1
            bload=bedrock_adjustment(ctr,par,load0,MASK,Hn,B,SLR,Db,VM, ...
...
%-----------------------------------------------------------------
    ...
    if ctr.inverse>=1 && rem(cnt*ctr.dt,ctr.Tinv)==0 && cnt>1 && ...
        ...
        [As,deltaZ,Asor]=optimize_ice_sheet(ctr,par,cnt, ...
    ...
    if ctr.inverse==2 && rem(cnt*ctr.dt,ctr.TinvMelt)==0
        if ctr.GroundedMelt==1
            [MeltInv]=optimize_ice_shelf(ctr,MASKo,glMASK,H,Ho,Melt,bMASK);
        else
            [MeltInv]=optimize_ice_shelf(ctr,MASK,glMASK,H,Ho,Melt,bMASK);
        end
    end
    if ctr.inverse>=1
        ...
        if ctr.shelf==1
            InvVol(cnt,3)=nanmean(H(shMASK==1)-Ho(shMASK==1)); %VL: add mean shelf thickness misfit
        end
    end
...
%------------------------------------------------------
    ...
    [VAF(cnt),POV(cnt),SLC(cnt),VA0(cnt)]=sea_level(ctr,par,SLR,B,H,H0,VAF0,POV0);
    if ctr.GeoidCalc==1 && cntT==1
        [SLR,dSLR]=calculate_geoid(par,ctr,fc.DeltaSL(cnt),SLC(cnt),SLR,SLR0, ...
...
%------------------------------------
    ...
    if ctr.runmode<2 && rem(cnt-1,plotst)==0
        plot_main_figure(ctr,par,x,y,sn,S0,H,u,B,MASK,glMASK,LSF);
    end
...
%--------------------------
if ctr.runmode<2
    final_plots(ctr,outfile,time,InvVol,SLC,u,v,MASK,bMASK,glMASK);
end
...
function [Tsf,Mbf,Prf,Evpf,runofff,cnt_atm,snp_atm,Mb_update]=ATM_update(fc,ctr,time,cnt,Ts0,Mb0,Pr0,Evp0,runoff0,cnt_atm,snp_atm,Mb_update,Tsf,Mbf,Prf,Evpf,runofff)

    if any(ismember(fields(fc),'atm_Ts_fname')) || any(ismember(fields(fc),'atm_Mb_fname')) || any(ismember(fields(fc),'atm_Pr_fname')) || any(ismember(fields(fc),'atm_Evp_fname')) || any(ismember(fields(fc),'atm_runoff_fname'))
...
% Advecting the grounding line thickness following Pelle et al. (2018)
% Employs upstream difference scheme
...
%VL: calculate large scale bed variability for ncor
...
function [eta,dudx,dvdy,dudy,dvdx]=EffVisc(A,uxssa,uyssa,H,par,eta1,MASK,ctr) %VL: additional output
...
% Interpolate Arc between consecutive calls (to improve
% calculation speed)
...
function Melt=ISMIP6_ocean(par,ctr,ice_draft,icemask_shelves,TF_draft,basinNumber,deltaT_basin)
% Sainan: ISMIP6 ocean parameterization equation (1)
    ...
    for i=1:Nbasin
        mean_TF(i)=mean(TF_draft(basinNumber==i & icemask_shelves>5.e-1 & ice_draft<=0.));
    end
    ...
    for i=1:Nbasin
        Melt(basinNumber==i)=ctr.gammaT.*cste.*(TF_draft(basinNumber==i)+deltaT_basin(basinNumber==i))...
            .*abs(mean_TF(i)+deltaT_basin(basinNumber==i));
    end
    Melt(TF_draft<=-5)=0;
    Melt=Melt*par.rhof/par.rho;
end

function Melt=ISMIP6_ocean_slope(sina,par,ctr,ice_draft,icemask_shelves,TF_draft,basinNumber,deltaT_basin)
% Sainan: ISMIP6 ocean parameterization equation (1) + adapted by Vio to
% take the local slope into account
    ...
    for i=1:Nbasin
        Melt(basinNumber==i)=ctr.gammaT.*cste.*(TF_draft(basinNumber==i)+deltaT_basin(basinNumber==i))...
            .*abs(mean_TF(i)+deltaT_basin(basinNumber==i)).*sina((basinNumber==i));
    end
    Melt(TF_draft<=-5)=0;
    Melt=Melt*par.rhof/par.rho;
end
...
%--------------------------------------
% Surface mass balance parametrization
%--------------------------------------
...
function [To,So,TF]=OCEAN_depth_of_int(fc,ctr,par,Tof,Sof,TFf,H,HB,B,MASK,glMASK,ShelfN,numsh)
    if any(ismember(fields(fc),'z')) % Depth-profile of To and So data or TF
        if ctr.meltfunc==3 || ctr.meltfunc==4 
            ...
            for b=1:numsh
                ...
                if isnan(avg_B)==1
                    avg_B=mean(B(ShelfN==b)); % some small shelves have front not well define, then average over shelf area
                end
                front_bot_depth_avg(ShelfN==b)=avg_B;
            end
            depth_of_int=front_bot_depth_avg;
            To=interp_to_depth_of_int(Tof,fc.z,depth_of_int,ctr);
            So=interp_to_depth_of_int(Sof,fc.z,depth_of_int,ctr);
            if islogical(TFf)==0
                TF=interp_to_depth_of_int(TFf,fc.z,depth_of_int,ctr);
            else
                TF=TFf;
            end
        else
            % compute deepest entrance depth of continental shelf for each ice shelf (Burgard et al. 2021 -- PROTECT)
            front_bot_depth_max=zeros(ctr.imax,ctr.jmax)-2500;
            for b=1:numsh
                max_B=min(B(ShelfN==b & glMASK==5));
                if isempty(max_B)==1
                    max_B=min(B(ShelfN==b)); % some small shelves have front not well define, then average over shelf area
                end
                front_bot_depth_max(ShelfN==b)=max_B;
            end
            depth_of_int=HB; % interpolate at ice draft
            depth_of_int(HB<front_bot_depth_max)=front_bot_depth_max(HB<front_bot_depth_max); % Limit depth of int to the deepest entrance depth of continental shelf
            if islogical(TFf)==0
                TF=interp_to_depth_of_int(TFf,fc.z,depth_of_int,ctr);
                To=Tof;
                So=Sof;
        ...
        if (ctr.meltfunc==3 || ctr.meltfunc==4) && any(ismember(fields(fc),'PICO_basins')) 
            % PICO method of updating values by basin (Kreutzer et al. 2021)
            [contshelfMASK]=continental_shelf_MASK(MASK,H,B,par); % update continental shelf mask
            SO=MASK*0;
            TO=MASK*0;
            for b=1:19
...
function [Tof,Sof,TFf,cnt_ocn,snp_ocn]=OCEAN_update(fc,ctr,time,cnt,So0,To0,Tof,Sof,TFf,cnt_ocn,snp_ocn)
    ...
    if any(ismember(fields(fc),'ocn_TF_fname')) || any(ismember(fields(fc),'ocn_To_fname')) || any(ismember(fields(fc),'ocn_So_fname')) 
...
function [runoff,acc,rain,Smelt]=PDDmonthly(Ts_yc,Pr_yc,par,ctr)
    ...
    T=(par.Tsigma/sqrt(2*pi)*exp(-(nT.^2))+0.5*(Tm-par.PDDth).*erfc(-nT))+par.PDDth;

    % Rain fraction calculation
    nP=(Tm-par.Tsnow)/(sqrt(2)*par.Psigma);
    P=(par.Psigma/sqrt(2*pi)*exp(-(nP.^2))+0.5*(Tm-par.Tsnow).*erfc(-nP))+par.Tsnow;
    ...
    for y=1:nyear % run through several years to equilibrium (Tsai 2020)
        ...
        for nstep=1:par.PDDsteps
            ...
            acc_nstep=max(0,PR.*(1-R(:,:,nstep))); % Total Snow Accumulation during nstep. R is the fraction of precipitation that falls as rain
            rain_nstep=max(0,PR.*R(:,:,nstep)); % Total Rain during nstep
            snow_depth=max(0,snow_depth+acc_nstep);
            ESM_nstep=min(max(snow_depth,0),par.snowfac.*PDD(:,:,nstep)); % Effective snow melt (cannot exceed the amount of snow) in m ice equivalent
            PDDr=max(PDD(:,:,nstep)-ESM_nstep./par.snowfac,0); % remaining PDDs after snow melt
    ...
    % Refreezing model
    W_r=ESM+rain; % Available water mass - maximum amount of water that can be retained provided enough energy is available
    ...
    E_r=min(Pr,min(P_r,W_r)); % Effective Refreezing - amount of water that is effectively retained at the end of the melting season
    % Er is limited by the annual precipitation
    runoff=EIM+W_r-E_r; % ice melt + rain and snowmelt liquid water beyond the snowpack saturation
end
...
function [runoff,acc,rain,Smelt]=PDDyearly(ctr,par,Ts,Pr,lat,MASK,sn)
    ...
    t=repmat(reshape(linspace(1,365.25,par.PDDsteps),1,1,par.PDDsteps),[ctr.imax,ctr.jmax,1]);
    Tm=repmat(Ts,[1,1,par.PDDsteps])-0.5*repmat(Ta,[1,1,par.PDDsteps]).*sin(2*pi*t/365.25);
    % PDD calculation from Calov and Greve (2005)
    nT=(Tm-par.PDDth)/(sqrt(2)*par.Tsigma);
    T=(par.Tsigma/sqrt(2*pi)*exp(-(nT.^2))+0.5*(Tm-par.PDDth).*erfc(-nT))+par.PDDth;
    ...
    % Melt model
    acc=max(0,Pr.*(1-R)); % Snow Accumulation Rate. R is the fraction of precipitation that falls as rain
    rain=max(0,Pr.*R);
    ESM=min(max(acc,0),par.snowfac.*PDD); % Effective snow melt (cannot exceed the amount of snow)
...
% Extrapolate melt underneath each ice shelf based on the results for each
% ocean box
    ...
    g1=Ak*ctr.gammaT;
    for i=1:numsh
        Tref=min(par.lambda1*S0o(i)+par.lambda2+par.lambda3*HB-T0o(i),0); % make sure Tref<=0
        dif=g1./(ctr.C*par.rhoref*(par.betao*S0o(i)/nu-par.alphao));
        x=-0.5*dif+0.5*sqrt(dif.^2-4*dif.*Tref);
        Tsh=T0o(i)-x; % T1 should be lower than T0
        Ssh=S0o(i)*(1-x/nu);
        m=-ctr.gammaT*(par.lambda1*Ssh+par.lambda2+par.lambda3*HB-Tsh)/nu*par.secperyear;
...
% LZ: function PICOsetup changed to seperate shelf numbering from PICOsetup
function [Bk,Ak,Bmax]=PICOsetup(glMASK,H,ctr,par,ShelfN,numsh,shMASK)

% Setup of the PICO model to determine each ice shelf (ShelfN), its area
...
x_hat = compute_X_hat(ice_draft_depth,zGL,T_in,Tf,E0,c_tau,gamma,sina,par);

M_hat = compute_M_hat(x_hat);

Mterm = compute_Mterm(T_in, S_in, Tf, c_rho_1, c_tau, gamma, E0, thermal_forcing, sina, par);
...
function [d,udx,udy,ud,ubx,uby,ub,uxsia,uysia,p,pxy]= ...
    ...
    if ctr.Tcalc==2
        p=par.n-1+par.Q1*G.*Hm./(par.K*par.R.*(h2d(Tb)+par.T0).^2); % on staggered d-grid
        pxy=par.n-1+par.Q1*G.*H./(par.K*par.R.*(Tb+par.T0).^2); % on h-grid     
        px=par.n-1+par.Q1*G.*Hmx./(par.K*par.R.*(0.5*(Tb+circshift(Tb,[0 -1]))+par.T0).^2); % on h-grid     
        py=par.n-1+par.Q1*G.*Hmy./(par.K*par.R.*(0.5*(Tb+circshift(Tb,[-1 0]))+par.T0).^2); % on h-grid     
    end
    if ctr.u0>1e10
        ds=1; % disable correction when u0 is default value
    else
        ds=max(0.01,min(1-Asfd./ctr.u0.*taud.^ctr.m,1)); % Zoet correction factor %VL: use Asfd
    end
    d=par.Z*Ad./(p+2).*Hm.^(par.n+2.).*gradm.^((par.n-1.)/2.)+ ...
        Asfd*(par.rho*par.g)^ctr.m.*Hm.^(ctr.m+1.).*gradm.^((ctr.m-1.)/2.)./ds; %VL: use Asfd
    udx=par.Z*Ax./(px+2).*signx.*Hmx.^(par.n+1.).*gradmx.^(par.n/2.);
    udy=par.Z*Ay./(py+2).*signy.*Hmy.^(par.n+1.).*gradmy.^(par.n/2.);
    ud=par.Z*A./(pxy+2).*H.^(par.n+1.).*gradxy.^(par.n/2.); % only used for temperature (on h-grid)
    ubx=Asfx.*signx.*(par.rho*par.g*Hmx).^ctr.m.*gradmx.^(ctr.m/2.); % only for temperature
...
function [MASKmx,MASKmy,HAFmx,HAFmy]=SSA_masks(ctr,par,SLR,Bmx, ...
...
function [uxssa,uyssa,beta2,eta,dudx,dudy,dvdx,dvdy,su,ubx,uby,ux,uy,damage]= ...
    ...
    if par.ShelfPinning==1 && ctr.stdBexist==1 && ctr.inverse==0 
        fg=max(0,1-(HB-B)./stdB); % subgrid pinning points in ice shelf  %VL: fg on h-grid
    else
        fg=1;
    end
    ussa=max(ussa,1e-3); % lower limit to prevent absence of sliding in ideal conditions
    if ctr.u0>1e10
        beta2=fg.*(ussa.^(1/ctr.m-1)).*Asf.^(-1/ctr.m);
    else
        beta2=fg.*(ussa.^(1/ctr.m-1)).*((ussa+ctr.u0).*Asf/ctr.u0).^(-1/ctr.m);
    end
    beta2=min(beta2,1e8);
    beta2(MASK==0)=0;
    ...
    eta1=0.5*Hshelf.*Tf.^(1.-par.n)./A; % Viscosity on edge (or when viscosity cannot be calculated)
    ...
    for ll=1:par.visciter % iteration over effective viscosity
        [eta,dudx,dvdy,dudy,dvdx]=EffVisc(A,uxssa,uyssa,H,par,eta1,MASK,ctr); %VL: additional output for Schoof
        ...
        %VL: also return iteration number and flag to monitor numerical stability
        [uxs1,uys1,su]=StreamStag_opt(nodeu,nodev,su,MASKmx,MASKmy,bMASK, ...
            H,eta,betax,betay,uxssa,uyssa,uxsia,uysia,udx,udy,taudx,taudy,ctr,par); %FP add ud,vd           
    ...
    uxssa=min(max(-par.maxspeed,uxssa),par.maxspeed);   %LZ2021
    uyssa=min(max(-par.maxspeed,uyssa),par.maxspeed);   %LZ2021
    ux=uxssa;   %LZ2021
    uy=uyssa;   %LZ2021
...
% LZ: new function Shelfsetup to seperate shelf numbering from PICOsetup
...
%--------------------------------------
% Solving SSA equation
%--------------------------------------
function [u,v,s]=StreamStag_opt(nodeu,nodev,s0,MASKmx,MASKmy,bMASK, ...
...
U0(MASKb==1)=-5.*(eta1(MASKb==1)+eta(MASKb==1))/(ctr.delta^2)- ...
    betax(MASKb==1);
U1(MASKb==1)=2.*(eta1(MASKb==1)+eta(MASKb==1))/(ctr.delta^2)+2.*dmudx(MASKb==1)/ctr.delta;
U2(MASKb==1)=2.*(eta1(MASKb==1)+eta(MASKb==1))/(ctr.delta^2)-2.*dmudx(MASKb==1)/ctr.delta;
U3(MASKb==1)=0.5*(eta1(MASKb==1)+eta(MASKb==1))/(ctr.delta^2)+0.5*dmudy(MASKb==1)/ctr.delta;
U4(MASKb==1)=0.5*(eta1(MASKb==1)+eta(MASKb==1))/(ctr.delta^2)-0.5*dmudy(MASKb==1)/ctr.delta;
Uv0(MASKb==1)=-1.5*(eta(MASKb==1)+eta1(MASKb==1))/(ctr.delta^2)+(dmudx(MASKb==1)-0.5*dmudy(MASKb==1))/ctr.delta;
Uv1(MASKb==1)=1.5*(eta(MASKb==1)+eta1(MASKb==1))/(ctr.delta^2)+(dmudx(MASKb==1)+0.5*dmudy(MASKb==1))/ctr.delta;
Uv2(MASKb==1)=1.5*(eta(MASKb==1)+eta1(MASKb==1))/(ctr.delta^2)-(dmudx(MASKb==1)+0.5*dmudy(MASKb==1))/ctr.delta;
Uv3(MASKb==1)=-1.5*(eta(MASKb==1)+eta1(MASKb==1))/(ctr.delta^2)-(dmudx(MASKb==1)-0.5*dmudy(MASKb==1))/ctr.delta;
...
if ctr.shelf==1 && ctr.mismip==0
    ...
    R0(MASKb==1)=0.5*par.rho*par.g*H1(MASKb==1).^2.*(1.-par.rho/par.rhow);
...
V0(MASKb==1)=-5.*(eta1(MASKb==1)+eta(MASKb==1))/(ctr.delta^2)- ...
    betay(MASKb==1);
V1(MASKb==1)=2.*(eta1(MASKb==1)+eta(MASKb==1))/(ctr.delta^2.)+2.*dmudy(MASKb==1)/ctr.delta;
V2(MASKb==1)=2.*(eta1(MASKb==1)+eta(MASKb==1))/(ctr.delta^2.)-2.*dmudy(MASKb==1)/ctr.delta;
V3(MASKb==1)=0.5*(eta1(MASKb==1)+eta(MASKb==1))/(ctr.delta^2.)+0.5*dmudx(MASKb==1)/ctr.delta;
V4(MASKb==1)=0.5*(eta1(MASKb==1)+eta(MASKb==1))/(ctr.delta^2.)-0.5*dmudx(MASKb==1)/ctr.delta;
Vu0(MASKb==1)=-1.5*(eta(MASKb==1)+eta1(MASKb==1))/(ctr.delta^2)+(dmudy(MASKb==1)-0.5*dmudx(MASKb==1))/ctr.delta;
Vu1(MASKb==1)=1.5*(eta(MASKb==1)+eta1(MASKb==1))/(ctr.delta^2)+(dmudy(MASKb==1)+0.5*dmudx(MASKb==1))/ctr.delta;
Vu2(MASKb==1)=1.5*(eta(MASKb==1)+eta1(MASKb==1))/(ctr.delta^2)-(dmudy(MASKb==1)+0.5*dmudx(MASKb==1))/ctr.delta;
Vu3(MASKb==1)=-1.5*(eta(MASKb==1)+eta1(MASKb==1))/(ctr.delta^2)-(dmudy(MASKb==1)-0.5*dmudx(MASKb==1))/ctr.delta;
...
if ctr.ItSolv==1
    ...
    C1=tril(A);
    C2=D\triu(A);
    [s,flag]=bicgstab(A,R,par.veltol,par.veliter,C1,C2,s0);   %VL: also return iteration number; tolerance and max. iterations defined in par.
    if flag>0
        s=A\R;
    end
else
...
function [Melt,butfac,H]=SubShelfMelt(ctr,fc,par,Tf,To,So,TF,butfac, ...
    HB,glMASK,H,B,ShelfN,numsh,shMASK,MASK,MASKlk,uxssa,uyssa,arcocn,MeltInv,cnt)
    ...
    switch ctr.meltfunc
        ...
        case 22
            ...
            sina=compute_sin_slope(HB,uxssa,uyssa,ctr); 
            mean_TF=zeros(ctr.imax,ctr.jmax);
            for i=1:numsh
                mean_TF(ShelfN==i)=mean(Tfm(ShelfN==i));
            end
            Melt=ctr.gammaT*par.LatentMelt^2*abs(mean_TF).*(Tfm).*sina*par.secperyear;
        case {3,4}
            % PICO/PICOP
            ...
            [T,S]=BoxModel(S0o,T0o,numsh,par,Ak,Bk,ShelfN,HB,1/par.LatentMelt,ctr);
            if ctr.meltfunc==3
                % PICO
                Melt=PICOMelt(HB,shMASK,ShelfN,Ak,Bk,S0o,T0o,S,T,numsh,Bmax,ctr,1/par.LatentMelt,par);
            else
                % PICOP
        ...
        case 9
            ...
            Melt=ISMIP6_ocean(par,ctr,HB,shMASK,Tfm,fc.basinNumber,fc.deltaT_basin);
        case 91
            % ISMIP6 non-local melt rate parameterization + dependy on local slope
            sina=compute_sin_slope(HB,uxssa,uyssa,ctr); 
            Melt=ISMIP6_ocean_slope(sina,par,ctr,HB,shMASK,Tfm,fc.basinNumber,fc.deltaT_basin);
        case 92
            ...
            Melt=ISMIP6_ocean_slope(sina,par,ctr,HB,shMASK,Tfm,fc.basinNumber,deltaT_basin);
        case 10
    ...
    switch ctr.meltfunc
        case {1,2,3,4,5,9}
            ...
            if ctr.meltfac>0
                Melt=Melt*ctr.meltfac; % FP: add meltfac to increase melt
            end
    end
...
gdsx=-par.rho*par.g*(circshift(H,[0 -1])-circshift(H,[0 1]))/(2*ctr.delta)- ...
    par.rhof*par.g*(circshift(HB,[0 -1])-circshift(HB,[0 1]))/(2*ctr.delta);
gdsy=-par.rho*par.g*(circshift(H,[-1 0])-circshift(H,[1 0]))/(2*ctr.delta)- ...
    par.rhof*par.g*(circshift(HB,[-1 0])-circshift(HB,[1 0]))/(2*ctr.delta);
...
for i=1:ctr.imax-1
    for j=1:ctr.jmax-1
        if MASK(i,j)==1
            flux=dparea_war_gds(i,j,MASK,Bmelt, ...
...
flw(MASK==0)=0;
Wd=min(par.Wdmax,max(par.Wdmin,(12*par.waterviscosity*flw/mean(gdsmag(MASK==1))).^(1/3)));
end
...
%--------------------------------------
% Surface temperature parametrization
%--------------------------------------
...
function Bmelt=basal_melting(ctr,par,G,taudxy,ub,H,tmp,dzm,MASK)
...
function [Asf,Asfx,Asfy,Asfd,Tbc,Neff,pwv,Wtil,r]=basal_sliding(ctr,par, ...
    As,Tb,H,B,MASK,Wd,Wtil,Bmelt,flw,bMASK,bMASKm,bMASKx,bMASKy)
    ...
    Asf=par.AsScale*((1-r).*par.AsFroz+As.*r).*expflw./((Neff.^ctr.p)/(par.NeffScale^ctr.p));
...
function bload=bedrock_adjustment(ctr,par,load0,MASK,Hn,B,SLR,Db,VM,node,nodes,frb,kei,Ll)

    if ctr.Dbexist==1
        % variable flexural rigidity (Kevin)
        loadB=zeros(ctr.imax,ctr.jmax);
        loadB(MASK==1)=par.rho*par.g*Hn(MASK==1)-load0(MASK==1);
        loadB(MASK==0)=(-par.rhow)*par.g*(B(MASK==0)-SLR(MASK==0))-load0(MASK==0); % Takes into account geoid changes: !!! If GeoidCalc=1 & DeltaSL~=0, need to get rid of DeltaSL signal --> TO IMPLEMENT 
...
function [B0,load0,frb,kei,Ll]=bedrock_init(ctr,par,H,B,SLR,MASK)
...
function [arcocn,distocn,distgl]=calcOceanArc(io,jo,MASK,imax,jmax,delta,narc,tanarc,ifi,idir)
...
function [SLR,dSLR]=calculate_geoid(par,ctr,DeltaSL,SLC,SLR,SLR0,Bn,Hn,Pg0,frg,geoide)
    ...
    Pg1(MASKn==0)=(-par.rhow)*(Bn(MASKn==0)-SLR(MASKn==0)+DeltaSL)*ctr.delta^2.; % Takes into account previous geoid change but not DeltaSL (external SL forcing) as ocean mass
...
%--------------------------------------
function zeta=calculate_zeta(kmax,min_layer)
...
function [CMB,LSF,he]=calving_algorithms(ctr,par,dudx,dvdy,dudy,dvdx,glMASK,H,A, ...
    uxssa,uyssa,arcocn,B,runoff,MASK,MASKo,Ho,bMASK,LSF,node,nodes,VM)
...
function [M_hat]=compute_M_hat(X_hat)
...
function [Mterm] = compute_Mterm(T_in, S_in, Tf, c_rho_1, c_tau, gamma, E0, thermal_forcing_factor, sina, par)
...
Mterm = sqrt((par.beta_coeff_lazero.*S_in.*par.g) ./ (par.lambda3.*(par.Latent./par.cp0).^3)) .* sqrt(max(0,(1 - c_rho_1 .* gamma) ./ (par.Cd + E0.*sina))) .* ((gamma .* E0 .* sina) ./ (gamma + c_tau + E0.*sina)).^(3/2) .* (T_in - Tf) .* thermal_forcing_factor;

end

function [x_hat] = compute_X_hat(ice_draft_depth,zGL,T_in,Tf,E0,c_tau,gamma,sina,par)
    ...
    x_hat = par.lambda3 * (ice_draft_depth-zGL)./((T_in-Tf) .* (1 + par.C_eps_lazero .* ((E0 * sina)./(gamma + c_tau + E0 * sina)).^(3/4)));
...
c_rho_1 = (par.Latent * par.alpha_coeff_lazero) ./ (par.cp0 * gamma * par.beta_coeff_lazero .* S_in);
c_rho_2 = -par.lambda1 * par.alpha_coeff_lazero ./ par.beta_coeff_lazero;
c_tau = c_rho_2 ./ c_rho_1;

end

function [sina]=compute_sin_slope(HB,vx,vy,ctr)    % local basal slope alfa
...
function [contshelfMASK]=continental_shelf_MASK(MASK,H,B,par)
...
for i=1:max(max(contshelf))
    if sum(MASK1(contshelf==i)~=0)+sum(MASK2(contshelf==i)~=0)+sum(MASK3(contshelf==i)~=0)+sum(MASK4(contshelf==i)~=0)~=0
...
function [uxsch,uysch,d]=diffusive_correction(ctr,par,uxsch,uysch,udx,udy,d,Ad,p,Hm,gradm,bMASK)
    ...
    if ctr.SSA>0
        ...
        if ctr.schoof>=1
            ...
            d(gradm>1e-20)=dusch(gradm>1e-20).*Hm(gradm>1e-20)./sqrt(gradm(gradm>1e-20));
        end
...
function flux=dparea_war_gds(ewr,nsr,MASK,Bmelt,kegdsx,kegdsy, ...
                ...
                if count~=5
                    if i>=1 && i<ctr.imax && j>=1 && j<ctr.jmax
                        ...
                        if pp(par.dirpp_war(count))>0 && funcnt<=5e4
                            funcnt=funcnt+1;
                            flux=dparea_war_gds(i,j,MASK,Bmelt,kegdsx,kegdsy, ...
                                par,ctr,flux,gdsmag,funcnt);
...
function [ctr,invmax2D,Asor,ncor,To,So,Pr0,Evp0,runoff0,Evp,Hinit]= ...
    exist_params(ctr,par,ncor,Asor,stdB,v,uxssa,To,So,Db,B,MASK,As,Pr,Evp,runoff,Mb0,Hinit,Ho)
    ...
    if ctr.inverse>0
        ...
        if ctr.stdBexist==1
            [invmax2D,ncor]=init_optimization(ctr,par,ncor,MASK,B,stdB);
        else
...
function [Ts_yc,Pr_yc]=extract_annual_cycle(fc,ctr,par,Pr0,Ts0,S0,sn,MASK,lat,DeltaSL,snp_atm)
    if any(ismember(fields(fc),'atm_Ts_fname'))
        ...
        % If intra-annual Ts not provided, then yearly cycle is approximated -- correction for elevation changes already applied
        ...
        t=repmat(reshape(linspace(1,365.25,par.PDDsteps),1,1,par.PDDsteps),[ctr.imax,ctr.jmax,1]);
        Ts_yc=repmat(Ts0,[1,1,par.PDDsteps])-0.5*repmat(Ta,[1,1,par.PDDsteps]).*sin(2*pi*t/365.25);
    end
    if any(ismember(fields(fc),'atm_Pr_fname'))
        ...
        Pr_yc=Pr_yc+Pr0; % If intra-annual Pr not provided, then considered constant -- correction for elevation changes already applied
    end
end
...
function final_plots(ctr,outfile,time,InvVol,SLC,u,v,MASK,bMASK,glMASK)
        
    if ctr.inverse>=1
        figure('Name',['InvVol: ' outfile],'NumberTitle','off');    %VL: add figure name
    ...
    figure('Name',['SLC: ' outfile],'NumberTitle','off');   %VL: add figure name
    plot(time,SLC); grid on; 
    xlabel('Time (year)');
    ylabel('SLR contribution (m)');
    
    if ctr.vexist==1
        figure('Name',['Velocity: ' outfile],'NumberTitle','off');  %VL: add figure name
        if ctr.basin==1
...
function [sn,HB]=fixed_floatation(par,B,SLR,H,MASK)
...
function [uxsch,uysch]=grounding_line_flux(ctr,glMASK,HAF,B,SLR,par, ...
...
% + orientation of GL (applied for buttressing and schoof velocity) [Pollard & DeConto (2020)]
%   --> new call!

% Grounding line flux calculation according to Schoof following
% the method of Pollard and DeConto (2012)
...
[jGL,iGL] = meshgrid(1:ctr.jmax,1:ctr.imax); %VL: grid for GL orientation
    ...
    q0=(Ax*(par.rho*par.g)^(par.n+1)*(1-par.rho/par.rhow)^par.n.*Asfx.^(1/ctr.m)/4^par.n);
    qs=q0.^(ctr.m/(ctr.m+1));
    ...
    Mgl(hgx<0)=0;
    %VL: GL in x-direction, u>0 --> jg=jGL, jh=jg+1, ih=ig=iGL
    [angnorm(Mgl==1)]=arrayfun(@(iGL,jGL) normcalc(iGL,jGL,iGL,jGL+1,glMASK,ctr),iGL(Mgl==1),jGL(Mgl==1));  %VL
    nx(Mgl==1)=cos(angnorm(Mgl==1));
    ny(Mgl==1)=sin(angnorm(Mgl==1));
    Theta(Mgl==1)=max(min((butfac*(Txx1(Mgl==1).*nx(Mgl==1).^2+Tyy1(Mgl==1).*ny(Mgl==1).^2+Txy1(Mgl==1).*nx(Mgl==1).*ny(Mgl==1))+(1.-butfac)*Tf1(Mgl==1))./Tf1(Mgl==1),1),0);
    ...
    ugx(Mgl==1)=ugx(Mgl==1).*abs(nx(Mgl==1));   %VL
    ...
    % GL grid cell
    uxsch(Mgl==1)=ux(Mgl==1)+ncorx(Mgl==1).*wx(Mgl==1).*(qgx(Mgl==1)./max(Hmx(Mgl==1),0.01)-ux(Mgl==1));
    ...
    uxsch(Mgl==1)=ux(Mgl==1)+ncx(Mgl==1).*( ...
        sign(wx(Mgl==1)).*(1-wx(Mgl==1)).*(qgx(Mgl==1)./max(Hmx(Mgl==1),0.01)-ux(Mgl==1)) ... % wx>0 (retreat)
        +(1-sign(wx(Mgl==1))).*(min(qgx(Mgl==1)./max(Hmx(Mgl==1),0.01),ux1(Mgl==1))-ux(Mgl==1)));   % wx=0 (advance)
    ...
    [angnorm(Mgl==1)]=arrayfun(@(iGL,jGL) normcalc(iGL,jGL+1,iGL,jGL,glMASK,ctr),iGL(Mgl==1),jGL(Mgl==1));  %VL
    nx(Mgl==1)=cos(angnorm(Mgl==1));
    ny(Mgl==1)=sin(angnorm(Mgl==1));
    Theta(Mgl==1)=max(min((butfac*(Txx(Mgl==1).*nx(Mgl==1).^2+Tyy(Mgl==1).*ny(Mgl==1).^2+Txy(Mgl==1).*nx(Mgl==1).*ny(Mgl==1))+(1.-butfac)*Tf(Mgl==1))./Tf(Mgl==1),1),0);
    if ctr.schoof==1 % Schoof
        Theta=Theta.^(par.n*ctr.m/(ctr.m+1.));
    else % Pattyn
        Theta=Theta.^(par.n);
    ...
    uxsch(Mgl==1)=ux(Mgl==1)+ncx(Mgl==1).*( ...
        sign(wx(Mgl==1)).*(1-wx(Mgl==1)).*(qgx(Mgl==1)./max(Hmx(Mgl==1),0.01)-ux(Mgl==1)) ... % wx>0 (retreat)
        +(1-sign(wx(Mgl==1))).*(max(qgx(Mgl==1)./max(Hmx(Mgl==1),0.01),ux1(Mgl==1))-ux(Mgl==1)));   % wx=0 (advance)
    
    
    % GL flux in y-direction
    q0=(Ay*(par.rho*par.g)^(par.n+1)*(1-par.rho/par.rhow)^par.n.*Asfy.^(1/ctr.m)/4^par.n);
    qs=q0.^(ctr.m/(ctr.m+1));
    ...
    [angnorm(Mgl==1)]=arrayfun(@(iGL,jGL) normcalc(iGL,jGL,iGL+1,jGL,glMASK,ctr),iGL(Mgl==1),jGL(Mgl==1));  %VL
    nx(Mgl==1)=cos(angnorm(Mgl==1));
    ny(Mgl==1)=sin(angnorm(Mgl==1));
    Theta(Mgl==1)=max(min((butfac*(Txx1(Mgl==1).*nx(Mgl==1).^2+Tyy1(Mgl==1).*ny(Mgl==1).^2+Txy1(Mgl==1).*nx(Mgl==1).*ny(Mgl==1))+(1.-butfac)*Tf1(Mgl==1))./Tf1(Mgl==1),1),0);
    if ctr.schoof==1 % Schoof
    ...
    ugy(Mgl==1)=ugy(Mgl==1).*abs(ny(Mgl==1));   %VL
    ...
    % GL grid cell
    uysch(Mgl==1)=uy(Mgl==1)+ncory(Mgl==1).*wy(Mgl==1).*(qgy(Mgl==1)./max(Hmy(Mgl==1),0.01)-uy(Mgl==1));
    % ice shelf grid cell downstream of GL
    ...
    uysch(Mgl==1)=uy(Mgl==1)+ncy(Mgl==1).*( ...
        sign(wy(Mgl==1)).*(1-wy(Mgl==1)).*(qgy(Mgl==1)./max(Hmy(Mgl==1),0.01)-uy(Mgl==1)) ...   % wy>0 (retreat)
        +(1-sign(wy(Mgl==1))).*(min(qgy(Mgl==1)./max(Hmy(Mgl==1),0.01),uy1(Mgl==1))-uy(Mgl==1)));   % wy=0 (advance)
    ...
    [angnorm(Mgl==1)]=arrayfun(@(iGL,jGL) normcalc(iGL+1,jGL,iGL,jGL,glMASK,ctr),iGL(Mgl==1),jGL(Mgl==1));  %VL
    nx(Mgl==1)=cos(angnorm(Mgl==1));
    ny(Mgl==1)=sin(angnorm(Mgl==1));
    Theta(Mgl==1)=max(min((butfac*(Txx(Mgl==1).*nx(Mgl==1).^2+Tyy(Mgl==1).*ny(Mgl==1).^2+Txy(Mgl==1).*nx(Mgl==1).*ny(Mgl==1))+(1.-butfac)*Tf(Mgl==1))./Tf(Mgl==1),1),0);
    if ctr.schoof==1 % Schoof
    ...
    uysch(Mgl==1)=uy(Mgl==1)+ncy(Mgl==1).*( ...
        sign(wy(Mgl==1)).*(1-wy(Mgl==1)).*(qgy(Mgl==1)./max(Hmy(Mgl==1),0.01)-uy(Mgl==1)) ...   % wy>0 (retreat)
        +(1-sign(wy(Mgl==1))).*(max(qgy(Mgl==1)./max(Hmy(Mgl==1),0.01),uy1(Mgl==1))-uy(Mgl==1)));   % wy=0 (advance)
    
end


function [glMASK,MASK]=grounding_line_mask(MASK,bMASK,H,ctr)
...
if ctr.shelf==1
    ...
    if ctr.calving>=1
        glMASK(glMASK==4 & H<5)=6;  % sea ice/ocean (=6) FP: changed this 3/9/18 to remove icebergs
        MASK1=circshift(glMASK,[0 -1]); % glMASK(i,j+1)
        MASK2=circshift(glMASK,[0 1]); % glMASK(i,j-1)
        MASK3=circshift(glMASK,[-1 0]); % glMASK(i+1,j)
        MASK4=circshift(glMASK,[1 0]); % glMASK(i-1,j)
...
function [uxsch,uysch]=grounding_lines(ctr,par,glMASK,HAF,SLR,Ax,Ay,butfac, ...
    H,Hmx,Hmy,B,Bmx,Bmy,Asfx,Asfy,ux,uy,dudx,dvdy,dudy,dvdx,eta)
    ...
    [uxsch,uysch]=grounding_line_flux(ctr,glMASK,HAF,B,SLR,par,Ax,Ay, ...
...
function [ctr,fc]=init_ctr(ctr,fc,default)
    ...
    ctr.HinvMelt(any(ismember(fields(ctr),'HinvMelt'))==0)=default.HinvMelt;    %VL
    ctr.TinvMelt(any(ismember(fields(ctr),'TinvMelt'))==0)=default.TinvMelt;    %VL
    ctr.radnorm(any(ismember(fields(ctr),'radnorm'))==0)=default.radnorm;     %VL
    ...
    if any(ismember(fields(ctr),'shelftune'))==0    %VL: 2D shelftune
    ...
    ctr.ItSolv(any(ismember(fields(ctr),'ItSolv'))==0)=default.ItSolv;  %VL
...
function [Pg0,geoide,frg]=init_geoid(par,ctr,MASK,H0,B0,B,SLR)
...
function [snapshot,plotst,cnt_atm,snp_atm,cnt_ocn,snp_ocn,Mb_update,Li,Lj,dtdx,dtdx2,X,Y,x,y,MASK,H,Ho,B,Bo, ...
    MASKo,Mb,Ts,As,G,u,VAF,VA0,POV,SLC,Ag,Af,Btau, ...
    IVg,IVf,glflux,dHdt,time,mbcomp,InvVol,ncor,dSLR,SLR, ...
    Wd,Wtil,Bmelt,CMB,FMB,flw,p,px,py,pxy,nodeu,nodev,Tof,Sof,TFf,Tsf,Mbf,Prf,Evpf,runofff]=init_matrices(ctr,par,default,fc)
    ...
    %------------------------------
    [H,Ho,B,Bo,Mb,Ts,u,dSLR,CMB,FMB,Tof,Sof,TFf,Tsf,Mbf,Prf,Evpf,runofff]=deal(zeros(ctr.imax,ctr.jmax));
...
function [tmp,Tb,zeta,dzc,dzp,dzm]=init_temp_params(ctr,par,tmp,Ts)
    ...
    zeta=calculate_zeta(ctr.kmax,0.015);
...
function [var_int]=interp_to_depth_of_int(var,depth_levels,depth_of_int,ctr)
...
%VL: New function normcalc (to calculate direction of GL)
...
function [As,deltaZ,Asor]=optimize_ice_sheet(ctr,par,cnt, ...
    Asor,MASK,MASKo,bMASK,deltaZ,sn,sn0,r,ncor,B,stdB,vx,vy,ux,uy,invmax2D)

    %LZ: introduce deltaZold to compare deltaZ between consecutive iterations
    deltaZold=zeros(ctr.imax,ctr.jmax);     %LZ
    %VL: optimize only for grounded AND originally grounded grid cells (add "& MASKo==1")
    if cnt*ctr.dt>ctr.Tinv     %LZ
        deltaZold(MASK==1 & MASKo==1)=deltaZ(MASK==1 & MASKo==1);     %VL
    end      %LZ  
    deltaZ=zeros(ctr.imax,ctr.jmax);
    deltaZ(MASK==1 & MASKo==1)=max(-1.5,min(1.5,(sn(MASK==1 & MASKo==1)-sn0(MASK==1 & MASKo==1))/ctr.Hinv));
    if ctr.SlidAdjust==1 && ctr.Tcalc>0
        Asor(r>0 & abs(deltaZ)>=abs(deltaZold))=Asor(r>0 & abs(deltaZ)>=abs(deltaZold)).* ...
            10.^deltaZ(r>0 & abs(deltaZ)>=abs(deltaZold)); %LZ
    else
        Asor(abs(deltaZ)>=abs(deltaZold))=Asor(abs(deltaZ)>=abs(deltaZold)).* ...
            10.^deltaZ(abs(deltaZ)>=abs(deltaZold)); %LZ
    end
    Asor(Asor<par.invmin/10)=par.invmin/10;   % LZ: /10
    Asor(Asor>par.invmax*1000)=par.invmax*1000;   % VL: *1000
    Asor(Asor>invmax2D & ncor==0)=invmax2D(Asor>invmax2D & ncor==0);
    if ctr.vexist==1
        As = regularizationNew(Asor,B,stdB,vx,vy,ctr,ncor,par.stdDevRegul);  %VL: new regularization
    else
        As = regularizationNew(Asor,B,stdB,ux,uy,ctr,ncor,par.stdDevRegul);  %VL: new regularization
    end
    As(As<par.invmin)=par.invmin; % first limits (LZ)
...
function [Melt]=optimize_ice_shelf(ctr,MASKo,glMASK,H,Ho,Melt,bMASK)

    Ftan=1.725;
    Melt=Melt+Ftan.*tan(max(-1.3,min(1.3,(H-Ho)./ctr.HinvMelt))); %VL: smaller limit as Bernales (changes of Melt were too big for limit 1.5 --> created holes in Ross)
    Melt(MASKo==1)=0; %VL: No Melt for grounded grid cells - FP: change
    Melt=min(100,max(-100,Melt)); %VL: limit melt
    Melt(glMASK==6)=0;
    if ctr.basin==1
        Melt(bMASK==1)=0;
    end
...
function plot_main_figure(ctr,par,x,y,sn,S0,H,u,B,MASK,glMASK,LSF)
...
function [pot]=potential_filling(pot,ctr)
...
%VL: new regularization function
    ...
    for j=-nwin:nwin
        for i=-nwin:nwin
            ...
            if zdist<=span
                ...
                BedWeight=1./2.^(0.5*(max(1,log10(abs(circshift(B,[-i -j])-B)))-1));
                if ctr.stdBexist==1
                    stdBweight=1./2.^(max(1,log10(circshift(stdB,[-i -j])))-1);
                else
                    stdBweight=1;
                end
                DEMnew=DEMnew+GaussFiltWeight*VelWeight.*BedWeight.*stdBweight.*circshift(DEM,[-i -j]);
                TotWeightFac=TotWeightFac+GaussFiltWeight*VelWeight.*BedWeight.*stdBweight; %for normalization --> sum of all Probs=1
            end
        end
    end
    As=Asor;
...
function [VAF,POV,SLC,VA0]=sea_level(ctr,par,SLR,B,H,H0,VAF0,POV0)
    ...
    DENScorr=sum(sum((H0-H)*ctr.delta^2.))*((par.rho/par.rhow)-(par.rho/par.rhof))/par.Aoc;
...
%--------------------------------------
% Solving ice sheet equation
%--------------------------------------
function [H]=sparse_solver_new(node,nodes,Mb,H,B,SLR,MASK,dtdx,dtdx2, ...
    d,u,v,ctr,cnt,bMASK,VM,par)
%VL: also return iteration number and flag to monitor numerical stability

d=min(d,1e20); % FP: limit due to NaNs in initialization for high resolution
...
R0=Mb*ctr.dt+H-H*(1-par.omega).*V0-circshift(H,[0 -1])*(1-par.omega).*V1- ...
    circshift(H,[0 1])*(1-par.omega).*V2-circshift(H,[-1 0])*(1-par.omega).*V3- ...
    circshift(H,[1 0])*(1-par.omega).*V4- ...
...
if ctr.upstream==1
    R0=R0-circshift(H,[0 2])*(1-par.omega).*V5-circshift(H,[2 0])*(1-par.omega).*V6- ...
        circshift(H,[0 -2])*(1-par.omega).*V7-circshift(H,[-2 0])*(1-par.omega).*V8;
end
...
% Flow parmater A on different staggered grids. Originally, A is determined
% on the diffusivity grid (d-grid)
    ...
    Ad=h2d(A); % A on H-grid

end


function [bMASKm,bMASKx,bMASKy]=staggered_bMASK(ctr,bMASK)
...
%--------------------------------------
% Full englacial temperature calculation
%--------------------------------------
function [tmp,ctr]=temperature3d(tmp,Mb,Ts,pxy,par, ...
    ...
    l=sqrt(2*par.kdif*(H+1e-8)./max(Mb,1e-8)*par.secperyear);
    for i=1:length(ipos)
        tmp(ipos(i),jpos(i),:)=flip(Ts(ipos(i),jpos(i))+sqrt(pi)*0.5*l(ipos(i),jpos(i))* ...
            Tgrad(ipos(i),jpos(i))*(erf(zeta*H(ipos(i),jpos(i))/l(ipos(i),jpos(i)))- ...
...
function [A,Ax,Ay,Ad]=thermo_coupling(ctr,par,Tb,Tbc,H,bMASK,bMASKm,bMASKx,bMASKy)
...
function [FMB]=vertical_face_melt(ctr,par,SLR,B,Melt,MASK,glMASK,he)
