function [dtrs]=SurfaceTransportDamage(node,nodes,dtrs,Mb,H,MASK,dtdx,dtdx2, ...
...
% hence: Bar(H_i,j)=(H_i+1,j + H_i,j)/2. Similar for Damage!
    ...
    % Frank
    %V0=8*epsilon*dtdx+dtdx2*(u-um1+v-vm1)-max(Mb,0)*ctr.dt./max(H,1e-5); % i,j
    % jablasco
    V0=8*epsilon*dtdx+dtdx2*(u-um1+v-vm1)+max(Mb,0)*ctr.dt./max(H,1e-5); % i,j
    ...
    dtrs(node>0)=s(node(node>0));
    dtrs=max(0,min(H-eps,dtrs)); % FP: put limit on maximum dtr as 1/20 of ice thickness
