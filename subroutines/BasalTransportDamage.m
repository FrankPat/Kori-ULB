function [dtrb]=BasalTransportDamage(node,nodes,dtrb,Melt,KTerm,H,MASK,dtdx,dtdx2, ...
                                     u,v,ctr,cnt,bMASK,VM,par)

% Kori-ULB
% Sparse solver of damage transport (based on ice thickness solver)
% Basal transport based on Kachuck et al., (2022)
% DOI:0.1017/jog.2022.12
% OJO; Frank computes on velocity grids.
% hence: Bar(H_i,j)=(H_i+1,j + H_i,j)/2. Similar for Damage!
    ...
    % jablasco: Surface crevasses only incluse Surface mass balance
    % revisar signo basal melt
    % Frank
    %V0=8*epsilon*dtdx+dtdx2*(u-um1+v-vm1)-max(Melt,0)*ctr.dt./max(H,1e-5); % i,j
    % jablasco
    V0=8*epsilon*dtdx+dtdx2*(u-um1+v-vm1)+(max(KTerm,0)+max(Melt,0))*ctr.dt./max(H,1e-5); % i,j
    ...
    dtrb(node>0)=s(node(node>0));
    dtrb=max(0,min(H-eps,dtrb)); % FP: put limit on maximum dtr as 1/20 of ice thickness
